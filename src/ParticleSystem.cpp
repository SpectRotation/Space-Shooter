...
void CyclicForce::updateForce(Particle *particle) {
	...
	// Vector from the particle's position to the center
	ofVec3f toCenter = center - particle->position;
	ofVec3f tangentForce(toCenter.z, 0, -toCenter.x);

	// Normalize and scale by magnitude
	tangentForce = tangentForce.getNormalized() * magnitude * ofRandom(0.5, 3);

	particle->forces += tangentForce;
}