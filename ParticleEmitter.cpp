...
void ParticleEmitter::spawn(float time) {
	...
	case RadialEmitter:
	{
		ofVec3f dir = ofVec3f(ofRandom(-10, 10), ofRandom(-10, 10), ofRandom(-5, 5));
	...
	case RingEmitter:
	{
		ofVec3f dir = ofVec3f(ofRandom(-10, 10), ofRandom(-10, 10), ofRandom(-1, 1));
		float speed = 0.5* velocity.length();
		particle.velocity = dir.getNormalized() * speed;
		particle.position.set(position);
	}
		break;
